...
for good — a product shipped to strangers cannot re-grow a concept the operator retired on
their next init. Every default composition seeds GLOBAL (room_id NULL); migration 0070's
own backfill (room_id -> former_room_id on the 31 pre-existing scoped rows) is what stands
as the permanent record on installs that predate this — a fresh box never re-creates rooms
at all.
...
async def test_init_compositions_only_skips_canon(actions: Actions) -> None:
    ...
    a live DB without also paying for a canon re-ingest. Compositions still seed, global."""
