...
async def test_canon_recall_is_keyword_ranked_and_project_scoped(actions: Actions) -> None:
    ...
    src = "ref:sibling-one"
    h = await actions.create_or_find_object("Reference", "ref:sibling-one-history-homing", src)
    for name, val in (("name", "Homing instinct study"), ("topic", "sibling-one-history"),
    ...
    res = await run_spec(actions.pool, {"op": "function", "name": "canon", "args":
                         {"q": "frozen homing displacement baseline",
                          "project": "sibling-one"}}, None)
    ...
    canon = await run_spec(actions.pool, {"op": "function", "name": "canon", "args":
                           {"q": "generic join", "project": "sibling-one"}}, None)
