"""Boot heal — REBOOT SURVIVAL, the units half (thread 194eac83, operator ruling aaa8e841,
2026-09-13: "it has to survive and recover reboot by itself"). Run by osiris-boot-heal.service,
a oneshot ordered Before= every daemon unit at every boot: repairs the box's own installed
systemd units BEFORE anything depends on them, so a stub or a drifted unit (today's live
specimen — tests/test_cli.py's own deploy test once wrote 30-byte stubs through the real
installer with Path.home unpatched, and osiris-mcp/osiris-pulse came up dead on the next
reboot) never survives past this unit's own single boot-time pass.

NEVER A SECOND INSTALLER (Thoth's own boundary, DM 10245: "call his installer, don't fork
it"): reuses the SAME two sanctioned, already-idempotent install paths `osiris deploy`
itself calls — `src.cli._real_install_user_units` for the five deploy/user/*.service daemon
units, `scripts/install_prune_timers.sh` for the seven oneshot timer pairs under deploy/ —
never a third diff-and-copy mechanism invented here. Both are already compare-then-copy
(a byte-identical target is a silent no-op) and already refuse to write outside a real
`~/.config/systemd/user` unless explicitly redirected for a test.
